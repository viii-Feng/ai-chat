...
export function MaskAvatar(props: {
  ...
  if (props.modelAvatar) {
    return <Avatar modelAvatar={props.modelAvatar} />;
  }
  ...
  return (
    <div className={chatStyle["context-prompt-row"]}>
      ...
      <Input
        ...
        onInput={(e) => {
          if (props.prompt.content instanceof Array) {
            const newContent = JSON.parse(JSON.stringify(props.prompt.content));
            let modify = false;
            newContent.forEach((item: any) => {
              if (item.type == "text") {
                item.text = e.currentTarget.value as string;
                modify = true;
              }
            });
            if (!modify) {
              newContent.unshift({
                type: "text",
                text: e.currentTarget.value as string,
              });
            }
            props.update({
              ...props.prompt,
    ...
    props.updateContext((context) => {
      ...
      if (images.length > 0 || files.length > 0) {
        ...
        for (const file of files) {
          newContext.push({ type: "file", file });
        }

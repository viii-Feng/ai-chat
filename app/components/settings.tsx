  ...
  return (
    <>
      <List>
        ...
        <ListItem
          title={Locale.Settings.Sync.LocalDataSize}
          subTitle={Locale.Settings.Sync.LocalDataSizeTips}
        >
          <div style={{ display: "flex" }}>{dataSize}KB / 5120KB</div>
        </ListItem>
      </List>
...
export function Settings() {
  ...
  const hideMidjourneySetting = accessStore.hideMidjourneySetting;
  const hideSdSetting = accessStore.hideSdSetting;
  const [currentLang, setCurrentLang] = useState<string>(config.tts.lang);
  const { voices } = useSpeechSynthesis();
  const langs = useMemo(() => {
    const langArr = [] as string[];
    voices.forEach((v: any) => {
      if (
        !langArr.includes(v.lang) &&
        !v.lang.includes("liaoning") &&
        !v.lang.includes("shaanxi")
      ) {
        langArr.push(v.lang);
      }
    });
  ...
  const voiceOptions = useMemo(() => {
    return voices.filter((v: any) => v.lang.includes(currentLang));
  }, [voices, currentLang]);
  ...
  const uploadAvatar = async (event: any) => {
    ...
    // 先把图片上传到oss，如果上传成功，则使用url作为头像，否则压缩图片后作为头像
    let fileUrl = await uploadFileToFileServer(formData);
    if (!fileUrl) {
      fileUrl = await compressImage(file, 10 * 1000);
    }
    updateConfig((config) => (config.avatar = fileUrl));
  };
      ...
      <div className={styles["settings"]}>
        <List>
          <ListItem
            ...
            subTitle={Locale.Settings.UploadAvatar}
          >
            {/*<Popover*/}
          ...
          <ListItem title={Locale.Settings.Nickname}>
            <input
              type="text"
              ...
              onChange={(e) =>
                config.update(
                  (config) => (config.nickname = e.currentTarget.value),
                )
              }
            ></input>
          </ListItem>
          ...
          >
            ...
            {/*  />*/}
            {/*)}*/}
            {accessStore.hideUpdateLog ? (
              <></>
            ) : (
              <Link href={RELEASE_URL} target="_blank" className="link">
                {Locale.Settings.Update.UpdateLog}
              </Link>
            )}
          </ListItem>
        ...
        <List id={SlotID.CustomModel}>
          ...
          {!accessStore.hideUserCustomModel && (
            <ListItem
        ...
        {!hideMidjourneySetting && (
          <List>
            <ListItem
              title={`MidjourneyProxy ${Locale.Midjourney.Url}`}
              subTitle={Locale.Midjourney.SettingProxyCoverTip("URL")}
            >
              <input
                type="text"
            ...
            <ListItem
              title={`MidjourneyProxy ${Locale.Midjourney.Secret}`}
              subTitle={Locale.Midjourney.SettingProxyCoverTip("SECRET")}
            >
              <input
                type="text"
                value={accessStore.midjourneyProxySecret}
                onChange={(e) => {
                  accessStore.updateMidjourneyProxySecret(
        ...
        {!hideSdSetting && (
          <List>
            <ListItem
              title={`StableDiffusion ${Locale.StableDiffusion.Url}`}
              subTitle={Locale.StableDiffusion.SettingProxyCoverTip("BASE_URL")}
            >
              <input
                type="text"
                value={accessStore.sdProxyUrl}
                onChange={(e) => {
                  accessStore.updateSdProxyUrl(e.currentTarget.value);
                }}
              />
            </ListItem>
          </List>
        ...
        {/*<List>*/}
        {/*  <ListItem title={Locale.Settings.Speech.Lang}>*/}
        {/*    <Select*/}
        {/*      value={config.tts.lang}*/}
        {/*      onChange={(e) => {*/}
        {/*        updateConfig(*/}
        {/*          (config) => (config.tts.lang = e.target.value as string),*/}
        {/*        );*/}
        {/*        setCurrentLang(e.target.value as string);*/}
        {/*      }}*/}
        {/*    >*/}
        {/*      {Object.values(langs).map((v: any) => (*/}
        {/*        <option value={v} key={v}>*/}
        {/*          {v}*/}
        {/*        </option>*/}
        {/*      ))}*/}
        {/*    </Select>*/}
        {/*  </ListItem>*/}
        {/*  <ListItem title={Locale.Settings.Speech.Voice}>*/}
        {/*    <Select*/}
        {/*      value={config.tts.voice}*/}
        {/*      onChange={(e) => {*/}
        {/*        updateConfig(*/}
        {/*          (config) => (config.tts.voice = e.target.value as string),*/}
        {/*        );*/}
        {/*      }}*/}
        {/*    >*/}
        {/*      {Object.values(voiceOptions).map((v: any, index: number) => (*/}
        {/*        <option value={v.name} key={index}>*/}
        {/*          {v.name}*/}
        {/*        </option>*/}
        {/*      ))}*/}
        {/*    </Select>*/}

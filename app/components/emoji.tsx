...
import BaiChuanIcon from "../icons/baichuan.svg";
import ChatGlmIcon from "../icons/chatglm.svg";
import SparkIcon from "../icons/spark.svg";

import {
  MidjourneyIconBase64,
  StableDiffusionIconBase64,
  SunoIconBase64,
} from "@/app/constant";
...
export function Avatar(props: {
  ...
  if (props.modelAvatar) {
    return (
      <div className="no-dark">
        <img
          src={props.modelAvatar}
  ...
  if (props.model) {
    ...
    if (modelName == "midjourney") {
      return (
        <div className="user-avatar">
          <img src={MidjourneyIconBase64} className="user-avatar" />
        </div>
      );
    }
    if (modelName == "stable-diffusion") {
      return (
        <div className="user-avatar">
          <img src={StableDiffusionIconBase64} className="user-avatar" />
        </div>
      );
    }
    if (modelName.startsWith("qianfan")) {
      return (
        <div className="no-dark">
    ...
    if (modelName.startsWith("baichuan")) {
      return (
        <div className="no-dark">
          <BaiChuanIcon className="user-avatar" />
        </div>
      );
    }
    if (modelName.startsWith("spark")) {
      return (
        <div className="no-dark">
          <SparkIcon className="user-avatar" />
        </div>
      );
    }
    if (modelName.startsWith("suno")) {
      return (
        <div className="user-avatar">
          <img src={SunoIconBase64} className="user-avatar" />
        </div>
      );
    }
    if (modelName.startsWith("chatglm")) {
      return (
        <div className="no-dark">
          <ChatGlmIcon className="user-avatar" />
        </div>
      );
    }
  ...
  return (
    <div className="user-avatar">
      {props.avatar &&
        (props.avatar.startsWith("http") || props.avatar.length > 100 ? (

...
export function MessageExporter() {
  ...
  const selectedMessages = useMemo(() => {
    ...
    // 处理消息是对象的问题
    const newRet = JSON.parse(JSON.stringify(ret)) as ChatMessage[];
    newRet.forEach((msg: any) => {
      if (msg.content instanceof Array) {
        ...
        msg.content.forEach((item: any, index: number) => {
          if (item.type == "text") {
            ...
            content += `[![${index}](${item.image_url.url})](${item.image_url.url})`;
          } else if (
            item.type == "file" &&
            item.file.type.includes("image") &&
            item.file.url
          ) {
            content += `[![${index}](${item.file.url})](${item.file.url})`;
          } else if (item.type == "file") {
            content += `${item.file.url}` + "\n";
          }
        });
        msg.content = content;
      }
        ...
        <div className={styles["chat-info"]}>
          ...
          <div>
            <div className={styles["main-title"]}>
              {accessStore.appTitle
                ? accessStore.appTitle
                : "ChatGPT Next Web Pro"}
            </div>
            <div className={styles["sub-title"]}>
              {accessStore.appSubTitle
                ? accessStore.appSubTitle
                : Locale.Login.Tips + ". Power by AnnYun AI."}
            </div>

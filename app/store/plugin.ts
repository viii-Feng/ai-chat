...
export const usePluginStore = createPersistStore(
  ...
  }),
  {
    ...
    onRehydrateStorage: function (state) {
      ...
      const accessStroe = useAccessStore.getState();
      ...
      fetch("./plugins.json")
        ...
        .then((res) => {
          Promise.all(
            res.map((item: any) => {
              const pluginUrl = accessStroe.pluginProxyUrl
                ? accessStroe.pluginProxyUrl + item.schema
                : item.schema;
              ...
              return state.get(item.id)
                ? item
                : item.proxy
                  ? fetch(`/api/proxy/${p.pathname}?${p.search}`, {
                      headers: {
                        "X-Base-URL": p.origin,
                      },
                    })
